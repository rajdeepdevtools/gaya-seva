...
export default function UserManagementPage() {
  ...
  const handleDelete = (id: string, name: string) => {
    if (confirm(`Are you sure you want to delete user "${name}"? This CRUD action cannot be undone.`)) {
  ...
  const getRoleBadge = (role: UserAccount['role']) => {
    switch (role) {
      case 'SUPER_ADMIN':
        return <span className="px-2.5 py-0.5 rounded-md text-[10px] font-extrabold bg-purple-100 text-purple-900 border border-purple-300">👑 SUPER ADMIN</span>;
      case 'ADMIN':
        return <span className="px-2.5 py-0.5 rounded-md text-[10px] font-extrabold bg-indigo-100 text-indigo-800 border border-indigo-200">🛡️ ADMIN</span>;
      case 'PANDIT':
      ...
      default:
        return <span className="px-2.5 py-0.5 rounded-md text-[10px] font-extrabold bg-gray-100 text-gray-800 border border-gray-200">🙏 YATRI</span>;
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-6">
        <div>
          <span className="text-xs font-bold text-[#F58220] uppercase tracking-wider">USER ACCOUNTS &amp; ROLE MANAGEMENT</span>
          <h1 className="font-serif text-3xl font-bold text-[#4A2E1A] mt-1">GayaSeva Users &amp; Full CRUD Control</h1>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs font-medium">
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-gray-500 font-semibold block">Total Registered Users</span>
          <p className="text-2xl font-extrabold text-[#4A2E1A]">{users.length}</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-amber-700 font-semibold block">Teerth Pandits</span>
          <p className="text-2xl font-extrabold text-amber-600">{users.filter(u => u.role === 'PANDIT').length}</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-blue-700 font-semibold block">Taxi Drivers</span>
          <p className="text-2xl font-extrabold text-blue-600">{users.filter(u => u.role === 'DRIVER').length}</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-1">
          <span className="text-emerald-700 font-semibold block">Verified Pilgrims &amp; Hotels</span>
          <p className="text-2xl font-extrabold text-emerald-600">{users.filter(u => u.status === 'VERIFIED').length}</p>
        </div>
      </div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          title="Toggle Status (Verified / Suspended)"
                        >
                          Toggle Status
                        </button>
                        ...
                        <button
                          ...
                          title="Edit User Details (CRUD Update)"
                        >
      ...
      {/* CREATE & EDIT USER MODAL (CRUD CREATE & UPDATE) */}
          ...
          <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-2xl max-w-lg w-full space-y-6">
            <div className="flex justify-between items-center border-b pb-4">
              <h3 className="font-serif font-bold text-xl text-[#4A2E1A]">
                {editingUser ? 'Edit User Account (CRUD Update)' : 'Create New Account (CRUD Create)'}
              </h3>
            ...
            <form onSubmit={editingUser ? handleUpdate : handleCreate} className="space-y-4 text-xs">
              <div>
                ...
                <input 
                  ...
                  placeholder="e.g. Pandit Suresh Shastri"
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="e.g. Vishnupad Zone / Kolkata"
                  className="w-full px-3 py-2.5 border rounded-xl focus:outline-none focus:border-[#F58220]" 
                />
              </div>
