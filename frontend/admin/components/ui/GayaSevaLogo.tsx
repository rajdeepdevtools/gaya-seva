...
export const GayaSevaLogo: React.FC<GayaSevaLogoProps> = ({
  ...
  const numericSize = typeof size === 'number' ? size : parseInt(size.toString(), 10) || 40;
  ...
  return (
    <div className="flex items-center gap-3 group">
      ...
      <div className="flex flex-col">
        <span className={`font-serif text-xl font-bold tracking-tight ${textColor} group-hover:text-[#F6C343] transition-colors`}>
          Gaya<span className="text-[#F58220]">Seva</span>
        </span>
        <span className={`text-[10px] tracking-wider uppercase font-semibold ${subtextColor}`}>
          ADMIN PORTAL
        </span>
      </div>
    </div>
