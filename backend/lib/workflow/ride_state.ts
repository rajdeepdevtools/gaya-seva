...
export type UserRole = 'CUSTOMER' | 'DRIVER' | 'ADMIN' | 'SUPER_ADMIN' | 'SYSTEM';
 ...
 */
export const RIDE_TRANSITIONS: TransitionRule[] = [
 ...
 */
export function validateRideTransition(
  fromState: RideState,
  toState: RideState,
  role: UserRole
  ...
  const match = RIDE_TRANSITIONS.find(
    (rule) => rule.from === fromState && rule.to === toState
  );
